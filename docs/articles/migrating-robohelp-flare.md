...
5. Make sure the project folder is correct. This should be the parent folder inside which the new project folder will be created.
   ![Import Projet Wizard](../assets/article-images/import_project_wizard.png)
6. Click **Next**.
7. Select both **Convert all topics at once** and **Convert inline formatting to CSS styles**.
   ![Convert all topics at once and Convert inline formatting to CSS styles options](../assets/article-images/convert_css.png)

    !!! note
	    The second checkbox will create new styles in your CSS based on the inline formatting that _shouldn't_ exist in RoboHelp, but for sure _does_. This way, you can then analyze all the new styles that are created and either replace them with standard styles or remove them.
...
10. If errors occur during the import, Flare notifies you when it completes the process. It is recommended to save the log by clicking **Save Log** on the **Import Progress** window.
   ![Import warning](../assets/article-images/import_warning.png)
...
3. Go to the line indicated in the error and try to see what the problem is. 
   ![Identify topics imported with errors](../assets/article-images/find_import_error.png)
...
1. From the **Content Explorer**, delete the topic that you want to replace. In the **Link Update** window that is displayed, select **Do Not Remove Links**.
    ...
    !!! warning
        Keep in mind that if you _do_ choose to remove the links, Flare only removes the `<a href>` part. This means that `Click <a href="parameters.htm">Parameter Descriptions</a> for more information` becomes `Click Parameter Descriptions for more information`, with no link... which is obviously unhelpful (and won't show up in any reports, so it will be impossible to find and fix).
...
5. Clear the **Link generated files to source files** check box and click **Next**.
   ![Reimport file](../assets/article-images/reimport.png)
...
7. Select the **Import resources** option (**Keep existing structure** should be selected by default; if not, select that also). 
   ![Select folder and import resources](../assets/article-images/import_folder_resources.png)
8. Click **Finish**.
9. Flare displays a report of the imported files, including any resources, such as images. If they look OK, click **Accept**. The CSS is not applied, so the file will look a bit weird, but that's not a problem.
   ![Imported files report](../assets/article-images/accept_import.png)
10. Flare turns every newly imported file into a new TOC. In the **Project Organizer**, check the **TOC** folder and delete any extraneous TOCs (only keep the original one from RoboHelp).
11. Flare also creates some import files. In the **Project Organizer**, check the **Imports** folder and delete the entries.
...
2. Set **By Target = HTML5**. Note that in a larger module, this will freeze Flare for a while. Just wait a bit and the report will load.
   ![File issues viewer](../assets/article-images/file_issues_viewer.png)
...
Go into the **Content Explorer**, create the folders as described in the documentation standards and move each type of resource into its dedicated folder. If you are prompted to update links, do it. 

![Folder structure](../assets/article-images/folder_structure.png)
...
!!! note
	...
	![Toggle show files](../assets/article-images/toggle_show_files.png)